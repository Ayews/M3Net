    ...
    for i in range(0,num_gt):
        ...
        for j in range(0,num_rs_dir):
            ...
            try:
                ...
                if(tmp_mae == -1): print(gt_name,rs_dir_lists[j]+gt_name)
            except IOError:
    ...
    for i in range(0,num_gt):
        ...
        for j in range(0,num_rs_dir):
            ...
            try:
                ...
                if(pre[0] == -1): print(gt_name,rs_dir_lists[j]+gt_name)
...
def plot_save_pr_curves(PRE, REC, method_names, lineSylClr, linewidth, xrange=(0.0,1.0), yrange=(0.0,1.0), dataset_name = 'TEST', save_dir = './', save_fmt = 'pdf'):
    ...
    ## draw dataset name
    plt.text((xrange[0]+xrange[1])/2.0,yrange[0]+(yrange[1]-yrange[0])*4/7,dataset_name,horizontalalignment='center',fontsize=28, fontname='serif',fontweight='bold')
